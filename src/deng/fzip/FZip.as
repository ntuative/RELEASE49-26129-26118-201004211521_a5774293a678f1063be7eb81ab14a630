package deng.fzip
{
   ...
   public class FZip extends EventDispatcher
   {
      ...
      protected function progressHandler(param1:Event) : void
      {
         ...
         {
            close();
            if(!hasEventListener(FZipErrorEvent.const_582))
            {
               throw e;
            }
            dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_582,e.message));
         }
      }
      ...
      public function loadBytes(param1:ByteArray) : void
      {
         if(!var_90 && parseFunc == parseIdle)
         {
            ...
            {
               dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_582,"EOF"));
            }
         }
      }
      ...
      private function parseLocalfile(param1:IDataInput) : Boolean
      {
         if(var_490.parse(param1))
         {
            ...
            if(false)
            {
               var_132["null"] = var_490;
            }
            dispatchEvent(new FZipEvent(FZipEvent.const_1206,var_490));
            var_490 = null;
      ...
      {
         var _loc2_:* = null;
         if(var_45 != null && var_132 != null && param1 < var_45.length)
         {
