package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(var_1441 == RoomWidgetRoomQueueUpdateEvent.const_317)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_579);
         }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            var_1441 = param1.type;
            var_995 = false;
            var_994 = param1.position;
         }
         else
         {
            var_995 = true;
         }
         var_1754 = param1.hasHabboClub;
         var_1440 = param1.method_11;
         localizations.registerParameter("room.queue.position","position",var_994.toString());
         localizations.registerParameter("room.queue.position.hc","position",var_994.toString());
         localizations.registerParameter("room.queue.spectator.position","position",var_994.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",var_994.toString());
         showInterface();
      }
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(var_1441)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_317:
                  _loc1_.caption = !!var_1440 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = var_995;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_434:
                  _loc1_.caption = !!var_1440 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = var_995;
         }
         var _loc4_:IWindowContainer = _window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !var_1754;
         }
