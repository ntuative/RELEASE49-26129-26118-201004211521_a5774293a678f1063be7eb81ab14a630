package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_436 >= var_318.length)
         {
            return "";
         }
         var petColor:IPetColor = var_318[var_436] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_618[var_1551] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_436 = param1.index;
         if(var_436 < 0 || var_436 > var_318.length)
         {
            var_436 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_567.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_115,onTextWindowEvent);
         var_567 = new Map();
         ...
         var_897 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_318 = [];
         var_618 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_897);
               if(_loc11_)
               {
                  var_618 = _loc11_.breeds;
                  var_318 = _loc11_.colors;
                  if(var_318 == null)
                  {
                     var_318 = [];
                  }
         ...
         {
            _loc12_ = var_618[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_897,_loc12_.id),getRaceLocalizationKey(var_897,_loc12_.id)));
            _loc6_++;
         }
         var_567.add(var_897,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1009,onApproveNameResult);
         _loc5_ = var_567.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_318)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_567.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_436 >= 0 && var_436 < var_318.length)
         {
            _loc7_ = var_318[var_436] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_618[var_1551] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_897,_loc3_.id,_loc2_,AvatarScaleType.const_56);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_319,AvatarAction.const_567);
